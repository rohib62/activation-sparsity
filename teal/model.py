...
SUPPORTED_LAYERS = [LlamaDecoderLayer, MistralDecoderLayer, CohereDecoderLayer]
...
class SparseModelMixin:
    ...
    def _init_model_specific_attributes(self, model):

        """Initialize model-specific attributes like rotary embeddings if missing"""
        if "Cohere" in model.__class__.__name__ and not hasattr(model.model, 'rotary_emb'):
            model.model.rotary_emb = CohereRotaryEmbedding(config=model.config)
    ...
    def build_sparse_layers(self, histogram_path, grab_acts):
        ...
        for i, layer in enumerate(self.model.layers):
            if any(isinstance(layer, layer_type) for layer_type in SUPPORTED_LAYERS):
...
class CohereSparseConfig(CohereConfig):
    model_type = "cohere_sparse"
...
class CohereSparseForCausalLM(SparseModelMixin, CohereForCausalLM):
    # config_class = CohereSparseConfig
    config_class = CohereSparseConfig
...
# Function to handle all registrations in one place
def register_sparse_models():
    from transformers import AutoConfig, AutoModelForCausalLM
    
    AutoConfig.register("llama_sparse", LlamaSparseConfig)
    AutoConfig.register("mistral_sparse", MistralSparseConfig)
    AutoConfig.register("command_r_sparse", CohereSparseConfig)
