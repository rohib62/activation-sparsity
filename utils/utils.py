...
class ActivationModule:
    ...
    def grab_activations(self, x, key):
        print(f"Grabbing activations for {key}, shape: {x.shape}, mean: {x.mean():.3f}, std: {x.std():.3f}")
...
def get_sparse_model(model_name, device, histogram_path, **kwargs):
    """Get sparse model with consistent device handling.
    
    Args:
        model_name: Path or name of base model
        device: 'auto' for multi-gpu or specific device string
        histogram_path: Path to histogram files
    ...
    AutoConfig.register("command_r_sparse", CohereSparseForCausalLM)
    ...
    supported_classes = [
        ...
        "LlamaSparseForCausalLM", 
        "MistralSparseForCausalLM",
        "CommandRSparseForCausalLM",
        "CohereSparseForCausalLM"
    ]
    
    # Debug print
    class_name = get_model_class_name(model_name)
    print(f"Debug: Detected model class name: {class_name}")
    
    assert class_name in supported_classes, f"Model class name {class_name} not supported. Supported classes: {supported_classes}"

    
    # Select appropriate sparse model class
    if "Llama" in class_name:
        SparseModel = LlamaSparseForCausalLM
    elif "Mistral" in class_name:
        SparseModel = MistralSparseForCausalLM
    elif "Cohere" in class_name:  # Added this condition
        SparseModel = CohereSparseForCausalLM
    else:
    ...
    if device == 'auto':
        ...
        model_kwargs['device_map'] = None  # Don't use device map for single GPU
    else:
    ...
    # Remove trust_remote_code if it exists - will be handled by CommandRSparseForCausalLM
...
#     # Verify model class
#     class_name = get_model_class_name(model_name)
#     assert class_name in ["LlamaForCausalLM", "MistralForCausalLM", "LlamaSparseForCausalLM", "MistralSparseForCausalLM"], \
...
#         model_kwargs['device_map'] = 'auto'
#     elif device == 'cuda':
#         # For single GPU usage (e.g., in perplexity testing)
#         model_kwargs['device_map'] = None  # Don't use device map for single GPU
#     else:
#         model_kwargs['device_map'] = device
